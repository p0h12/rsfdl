...
graph LR
    ...
    UC18[UC-18: SFDL-Container erstellen]
    UC19[UC-19: Container verschlüsseln]
    User --> UC01
    User --> UC02
    User --> UC03
    User --> UC04
...
### Hauptablauf (CLI: `config show`)

1. Benutzer ruft `rsfdl-cli config show` auf
2. System lädt Einstellungen aus der Settings-Datei (oder Defaults)
3. System gibt alle Einstellungen inkl. Dateipfad auf stdout aus

### Hauptablauf (CLI: `config edit`)

1. Benutzer ruft `rsfdl-cli config edit` auf
2. System öffnet die Settings-Datei im Editor (`$EDITOR`, Fallback: `vi`/`notepad`)
3. Falls die Datei noch nicht existiert: System erstellt sie mit Default-Werten
4. Benutzer bearbeitet und speichert die Datei im Editor
5. System validiert die gespeicherte Datei (optional, bei nächstem Laden)
...
## UC-18: SFDL-Container erstellen

**Akteur**: Benutzer
**Vorbedingung**: FTP-Verbindungsdaten und Zielpfad(e) sind bekannt
**Requirements**: FR-20, FR-21, FR-22, FR-24, FR-25

### Hauptablauf (FileList-Modus)

1. Benutzer gibt FTP-Verbindungsdaten an (Host, Port, Username, Password)
2. Benutzer gibt einen Remote-Pfad an
3. System verbindet sich zum FTP-Server
4. System listet das Verzeichnis rekursiv auf
5. Für jede gefundene Datei wird ein FileItem erstellt (file_name, full_path, file_size, directory_root, directory_path)
6. System baut einen SfdlContainer mit Connection, Package (BulkFolderMode=false) und FileList
7. System setzt Metadaten (Description, Uploader, MaxDownloadThreads)
8. System serialisiert den Container als SFDL v3 XML
9. System schreibt die `.sfdl`-Datei

### Hauptablauf (BulkFolder-Modus)

1. Benutzer gibt FTP-Verbindungsdaten und Remote-Pfad an
2. System baut einen SfdlContainer mit Package (BulkFolderMode=true) und BulkFolderList
3. **Kein FTP-Connect nötig** — nur der Pfad wird gespeichert
4. System serialisiert und schreibt wie oben

### Alternativabläufe

- **A1**: FTP-Server nicht erreichbar (FileList-Modus) → Fehlermeldung "Connection failed: {details}"
- **A2**: Verzeichnis leer → Warnung, leerer Container wird trotzdem erstellt
- **A3**: Ausgabedatei existiert bereits → Überschreiben mit Warnung

---

## UC-19: Container verschlüsseln

**Akteur**: Benutzer
**Vorbedingung**: SfdlContainer ist aufgebaut (UC-18)
**Requirements**: FR-23

### Hauptablauf

1. Benutzer gibt ein Verschlüsselungspasswort an
2. System verschlüsselt alle sensitiven Felder mit AES-128-CBC:
    - Connection: Host, Username, Password
    - Container: Description, Uploader
    - Package: Name
    - FileItem: FileName, DirectoryRoot, DirectoryPath, FullPath, PackageName
    - BulkFolder: BulkFolderPath, PackageName
3. Jedes Feld erhält einen kryptographisch zufälligen IV
4. System setzt `Encrypted = true`
5. Port, FileSize, HashType bleiben unverschlüsselt

### Nachbedingung

- Der verschlüsselte Container kann mit demselben Passwort über UC-02 entschlüsselt werden

### Alternativabläufe

- **A1**: Kein Passwort angegeben → Container bleibt unverschlüsselt (Encrypted=false)

---

## Use Case Priorisierung

| Use Case                       | Prio   | Abhängig von |
|--------------------------------|--------|--------------|
| UC-01: SFDL öffnen             | MUST   | —            |
| UC-02: Passwort eingeben       | MUST   | UC-01        |
| UC-03: Inhalt anzeigen         | MUST   | UC-01/02     |
| UC-11: CLI Info                | MUST   | UC-01/02/03  |
| UC-05: Download starten        | MUST   | UC-03        |
| UC-06: Fortschritt             | MUST   | UC-05        |
| UC-07: Abbrechen               | MUST   | UC-05        |
| UC-08: Resume                  | MUST   | UC-05        |
| UC-12: CLI Download            | MUST   | UC-05        |
| UC-04: Dateien auswählen       | SHOULD | UC-03        |
| UC-09: Einstellungen           | SHOULD | —            |
| UC-13: Bandbreite begrenzen    | SHOULD | UC-05        |
| UC-14: Auto-Extraktion         | SHOULD | UC-05        |
| UC-15: Dateien ausschliessen   | SHOULD | UC-03        |
| UC-16: Speedreport             | COULD  | UC-05        |
| UC-17: Speicherplatz prüfen    | SHOULD | UC-05        |
| UC-18: Container erstellen     | MUST   | —            |
| UC-19: Container verschlüsseln | MUST   | UC-18        |
