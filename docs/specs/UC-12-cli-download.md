...
2. Entschlüsseln wenn nötig:
   a. -p angegeben → validate_password → decrypt_container
   b. Kein -p, aber password_list → try_passwords → decrypt_container
   c. Kein Passwort → Fehler "File is encrypted. Provide a password with -p"
3. AppSettings aus CLI-Args bauen:
   - dest → download_directory (Default: cwd)
   - threads → max_download_threads
   - Rest: AppSettings::default()
4. File/BulkFolder-Counts und Gesamtgrösse berechnen und ausgeben
5. DownloadManager::new(container, &settings) → (manager, cancel_token, _)
6. Ctrl+C Handler spawnen → cancel_token.cancel()
7. Progress-Channel erstellen
8. Manager in Background-Task spawnen
9. Progress-Event-Loop mit indicatif
10. Exit-Code: 0 bei Erfolg, 1 bei Fehlern
```
...
- Settings werden nicht aus JSON geladen, sondern aus CLI-Args konstruiert
