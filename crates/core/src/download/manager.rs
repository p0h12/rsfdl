    ...
    pub async fn run(
        ...
        for package in &self.container.packages {
            ...
            for file_item in &package.file_list {
                items.push(DownloadItem::from_file_item(
                    file_item,
                    &self.dest_dir,
                    &package.name,
                    self.create_package_subfolder,
                ));
            }

            // BulkFolder resolution (requires FTP connection)
            if package.bulk_folder_mode && !package.bulk_folder_list.is_empty() {
                match resolve_all_bulk_folders(
                    &self.container.connection,
                    &package.bulk_folder_list,
                    self.ftp_timeout_seconds,
                )
                .await
                {
                    Ok(resolved) => {
        ...
        let file_tokens: Arc<Mutex<HashMap<Uuid, CancellationToken>>> =
        ...
        for item in to_download {
            ...
            let file_cancel = global_cancel.child_token();
            {
                file_tokens
                    .lock()
                    .await
                    .insert(item.id, file_cancel.clone());
            }
