    ...
    fn from(err: suppaftp::FtpError) -> Self {
        match &err {
            suppaftp::FtpError::ConnectionError(_) => {
                FtpError::ConnectionFailed(err.to_string())
            }
            suppaftp::FtpError::SecureError(msg) => {
                FtpError::ConnectionFailed(format!("TLS error: {msg}"))
            }
            ...
            suppaftp::FtpError::BadResponse => {
                FtpError::TransferError("Bad FTP response".into())
            }
            suppaftp::FtpError::InvalidAddress(e) => {
                FtpError::ConnectionFailed(e.to_string())
            }
        }
    }
}
