...
pub fn extract_rar(main_file: &Path, dest_dir: &Path, mut on_progress: impl FnMut(u8)) -> Result<(), ExtractionError> {
	// First pass: count entries for progress calculation
	let entry_count = count_entries(main_file)?;

	// Second pass: extract
	let mut archive = Archive::new(main_file).open_for_processing().map_err(|e| map_unrar_error(&e))?;
