...
use detector::{detect_archives, ArchiveType, DetectedArchive};
...
mod tests {
    ...
    fn create_test_zip(dir: &Path, name: &str, files: &[(&str, &[u8])]) -> std::path::PathBuf {
        ...
        for (fname, content) in files {
            writer
                .start_file::<_, ()>(*fname, ::zip::write::SimpleFileOptions::default())
                .unwrap();
    ...
    async fn extract_archives_zip_sends_correct_event_sequence() {
        ...
        assert!(events
            .iter()
            .any(|e| matches!(e, ProgressEvent::ExtractionStarted { .. })));
        assert!(events
            .iter()
            .any(|e| matches!(e, ProgressEvent::ExtractionCompleted { .. })));
        assert!(events.iter().any(
            |e| matches!(e, ProgressEvent::ExtractionAllDone { extracted: 1, failed: 0, .. })
        ));
    }
    ...
    async fn extract_archives_zip_deletes_archive_when_configured() {
        ...
        assert!(dir.path().join("hello.txt").exists());
        assert!(
            !dir.path().join("files.zip").exists(),
    ...
    async fn extract_archives_invalid_zip_sends_failed_event() {
        ...
        assert!(events
            .iter()
            .any(|e| matches!(e, ProgressEvent::ExtractionFailed { .. })));
    }
    ...
    async fn extract_archives_failed_keeps_archive_even_with_delete() {
        ...
        assert!(
            dir.path().join("broken.zip").exists(),
            "broken archive should not be deleted"
        );
    }
