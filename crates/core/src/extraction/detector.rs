...
static PART_RAR_RE: LazyLock<Regex> =
...
static RAR_PART_RE: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"(?i)\.(rar|part\d+\.rar|r\d{2,3})$").unwrap());
...
fn rar_base_name(file_name: &str) -> Option<String> {
    ...
    if let Some(pos) = lower.rfind('.') {
        ...
        // .r00, .r01, etc.
        static R_NUM_RE: LazyLock<Regex> =
            LazyLock::new(|| Regex::new(r"(?i)\.r\d{2,3}$").unwrap());
    ...
    if let Ok(entries) = std::fs::read_dir(directory) {
        for entry in entries.flatten() {
            ...
            if let Some(entry_base) = rar_base_name(&name) {
                if entry_base.to_lowercase() == base {
...
mod tests {
    ...
    fn base_name_with_dots_in_name() {
        assert_eq!(
            rar_base_name("Movie.2026.720p.part01.rar"),
            Some("Movie.2026.720p".into())
        );
    }
    ...
    fn detect_archives_groups_multipart_rar() {
        ...
        let archives = detect_archives(dir.path());
        let rar_archives: Vec<_> = archives
            .iter()
            .filter(|a| a.archive_type == ArchiveType::Rar)
            .collect();
