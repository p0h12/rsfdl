...
use common::{
    create_ftp_file, generate_empty_sfdl_xml, generate_sfdl_xml, parse_sfdl_from_xml,
    FtpTestServer,
};
...
async fn download_multiple_files() {
    ...
    // Verify all files
    assert_eq!(
        fs::read(dest.path().join("TestPkg/data/file1.bin")).unwrap(),
        vec![0xAA; 512]
    );
    assert_eq!(
        fs::read(dest.path().join("TestPkg/data/file2.bin")).unwrap(),
        vec![0xBB; 1024]
    );
    assert_eq!(
        fs::read(dest.path().join("TestPkg/data/file3.bin")).unwrap(),
        vec![0xCC; 256]
    );
}
...
async fn download_creates_directory_structure() {
    ...
    let xml = generate_sfdl_xml(
        server.port(),
        &[("deep.bin", "a/b/c", "/a/b/c/deep.bin", 4)],
    );
    let container = parse_sfdl_from_xml(&xml);
...
async fn download_skip_complete() {
    ...
    let xml = generate_sfdl_xml(
        server.port(),
        &[("done.bin", "data", "/data/done.bin", content.len() as u64)],
    );
    let container = parse_sfdl_from_xml(&xml);
...
async fn download_cancellation() {
    ...
    let xml = generate_sfdl_xml(
        server.port(),
        &[("large.bin", "data", "/data/large.bin", content.len() as u64)],
    );
    let container = parse_sfdl_from_xml(&xml);
    ...
    assert!(
        result.cancelled >= 1 || result.completed >= 1,
        "file should be cancelled or have completed before cancellation"
    );
}
...
async fn download_parallel_threads() {
    ...
    let files: Vec<(String, String, String, u64)> = (0..5)
        .map(|i| {
            (
                format!("file{}.bin", i),
                "parallel".to_string(),
        ...
        .collect();
    let file_refs: Vec<(&str, &str, &str, u64)> = files
        .iter()
        .map(|(n, d, f, s)| (n.as_str(), d.as_str(), f.as_str(), *s))
        .collect();
    ...
    for i in 0..5 {
        let path = dest
            .path()
            .join(format!("TestPkg/parallel/file{}.bin", i));
