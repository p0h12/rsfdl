            ...
            if container.encrypted {
                ...
                if let Some(pw) = try_passwords(&container, &passwords) {
                    if let Err(e) = decrypt_container(&mut container, &pw) {
                        state.error_message.set(Some(format!("Auto-decrypt failed: {e}")));
                        return;
                    }
...
pub fn finish_container_load(state: &mut AppState, container: rsfdl_core::sfdl::models::SfdlContainer, path: String) {
    ...
    let selection: Vec<bool> = container.packages.iter()
...
async fn resolve_bulk_folders_async(state: &mut AppState) {
    let (conn, packages, timeout) = {
        ...
        (container.connection.clone(), container.packages.clone(), timeout)
    };
    ...
    match resolve_container_bulk_folders(&conn, &packages, timeout).await {
        Ok(resolved_files) => {
            if !resolved_files.is_empty() {
                ...
                let mut files_by_package: std::collections::HashMap<String, Vec<rsfdl_core::sfdl::models::FileItem>> = std::collections::HashMap::new();
