...
async fn create_sfdl(
    ...
    if host.is_empty() {
        state
            .error_message
            .set(Some("Host is required".to_string()));
        return;
    }
    if remote_path.is_empty() {
        state
            .error_message
            .set(Some("Remote path is required".to_string()));
        return;
    }

    // Derive package name from remote path
    let package_name = remote_path
        .trim_end_matches('/')
        .rsplit('/')
        .next()
        .unwrap_or("Package1")
    ...
    let packages = if bulk_folder_mode {
        ...
        match rsfdl_core::ftp::listing::resolve_container_bulk_folders(
            &conn,
            &[build_bulk_package(&remote_path, &package_name)],
            timeout,
        )
        .await
        {
            ...
            Err(e) => {
                state
                    .error_message
                    .set(Some(format!("FTP listing failed: {e}")));
                return;
            }
        }
    };
    ...
    let xml = match serialize_v3(&container) {
        ...
        Err(e) => {
            state
                .error_message
                .set(Some(format!("Serialization failed: {e}")));
            return;
        }
    };
    ...
    if let Err(e) = tokio::fs::write(file.path(), xml.as_bytes()).await {
        state
            .error_message
            .set(Some(format!("Failed to write file: {e}")));
        return;
    }
