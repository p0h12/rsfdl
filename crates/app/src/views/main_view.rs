...
use crate::state::{AppState, DownloadPhase, DownloadSummary, FileDownloadState, FileStatus, GlobalProgressState};
...
fn start_download(mut state: AppState) {
	...
	state.file_cancel_tx.set(Some(std::sync::Arc::new(file_cancel_tx)));
