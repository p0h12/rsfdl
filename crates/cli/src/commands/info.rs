...
pub fn run(args: &SfdlArgs, password_list: &[String]) {
	...
	let encrypted_label = match outcome {
		DecryptionStatus::NotEncrypted => "no",
		DecryptionStatus::AutoDecrypted { .. } => "yes (decrypted)",
		DecryptionStatus::NeedsPassword => "yes (not decrypted)",
	};
...
fn print_info(c: &SfdlContainer, encrypted_label: &str) {
	...
	println!("Server:    {}:{} ({}, {})", c.connection.host, c.connection.port, ssl_label, conn_label);
