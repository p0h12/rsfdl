...
pub async fn run(file: &str, password: Option<&str>, password_list: &[String], dest: Option<&str>, threads: u32, cli_exclude: &[String]) {
    ...
    let saved_settings = rsfdl_core::db::repository::load_settings(
        &rsfdl_core::db::repository::default_settings_path(),
    );
    ...
    if bulk_count > 0 {
        eprintln!(
            "Resolving {} bulk folder(s) via FTP...",
            bulk_count
        );
    }
    ...
    global_bar.set_style(
        ProgressStyle::with_template(
            "{prefix:.bold} [{bar:40.cyan/dim}] {bytes}/{total_bytes} {binary_bytes_per_sec} ETA {eta}",
        )
        .unwrap()
        .progress_chars("=>-"),
    );
    ...
    let file_style = ProgressStyle::with_template(
        "  {prefix:.cyan} [{bar:30.green/dim}] {bytes}/{total_bytes} {bytes_per_sec}",
    )
    .unwrap()
    .progress_chars("=>-");
    ...
    if settings.auto_extract_archives {
        ...
        let ext_handle = tokio::spawn(async move {
            rsfdl_core::extraction::extract_archives(&ext_dir, delete_after, &ext_tx).await
        });
        ...
        while let Some(event) = ext_rx.recv().await {
            match event {
                ...
                ProgressEvent::ExtractionAllDone {
                    ...
                    if total_archives > 0 {
                        eprintln!(
                            "Extraction: {} archives, {} extracted, {} failed",
                            total_archives, extracted, failed
                        );
                    }
                }
                _ => {}
