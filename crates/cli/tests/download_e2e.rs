...
use common::{create_ftp_file, generate_sfdl_xml, write_sfdl_to_file, FtpTestServer};
...
async fn cli_download_nonexistent_sfdl() {
    tokio::task::spawn_blocking(move || {
        cmd()
            .args(["download", "/tmp/nonexistent_12345.sfdl"])
            .assert()
            .failure();
    })
    .await
