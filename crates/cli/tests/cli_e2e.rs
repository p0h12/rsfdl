...
fn config_edit_creates_default_file() {
    ...
    cmd()
        .args(["config", "edit", "--config-file", &path.to_string_lossy()])
        .env("EDITOR", "true")
        .assert()
        .success();
...
fn download_override_does_not_modify_settings_file() {
    ...
    assert_eq!(
        ...
        "Settings file must not be modified by CLI overrides"
    );
}
...
fn info_uses_auto_password_list_from_settings() {
    ...
    cmd()
        ...
        .stderr(predicate::str::contains(
            "Auto-decrypted with password from list",
        ))
        .stdout(predicate::str::contains("ftp.example.com"));
}
